...
import { FetchKeywordSearchAttributesResults } from "../tools/RemoteData";
...
class KeywordSearchForm extends React.Component {
	...
	onSuggestionsFetchRequested = ({ value }) => {
		FetchKeywordSearchAttributesResults(value, [], (res) => {
			this.setState({
				suggestions: res,
			});
		});
	...
	onKeyPress = (event) => {
		if (event.key === "Enter") {
			...
			this.props.history.push(
				"/keyword-search?query=" + this.state.value
			);
		}
	};
	...
	render() {
		...
		const theme = {
			...
			suggestionsContainerOpen:
				"suggestions-container bg-white text-dark",
			suggestionList: "",
			suggestion: "p-2",
