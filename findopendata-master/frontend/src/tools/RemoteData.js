...
function FetchKeywordSearchAttributesResults(
	query,
	selectedOriginalHosts,
	callBack
) {
	...
	fetch(`${API_ENDPOINT}/keyword-search-attribute?${url_queries.join("&")}`)
		.then(handleErrors)
		.then((res) => res.json())
		.then((res) => {
			callBack(res);
...
function FetchSimilarPackageSearchResults(
	packageId,
	selectedOriginalHosts,
	callBack
) {
